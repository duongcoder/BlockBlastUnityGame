...
public class BlockSpawner : MonoBehaviour
{
    ...
    private IEnumerator HandlePlacedDelayed(int slotIndex, GameObject instance)
    {
        ...
        if (gridLogic != null)
        {
            ...
            Debug.Log($"[BlockSpawner] Checking game over with {blocks.Count} blocks in tray.");
            foreach (var b in blocks)
            {
                Debug.Log($"[BlockSpawner] tray block: {b.def.name} rot={b.rotationSteps}");
            }
            gridLogic.CheckGameOver(blocks);
        }
